...
- **Diagnostics** — immediate parse, incompatible-return/initializer/reassignment, undeclared-variable (`cannot find symbol` for a reference whose declaration was deleted or is out of scope), unreachable-statement, unused-code, and conservative unresolved-member feedback without waiting for `javac`, with type-mismatch errors carrying a clickable link to the declaration site; compiler diagnostics remain the automatic trusted-workspace backstop
...
- **Refactoring & code generation** — extract a selected expression to a local variable or a `private static final` constant, extract a run of statements to a new method, inline a local variable into its usages; generate getters/setters, an all-fields constructor, `equals()`/`hashCode()`, and `toString()` from the Source Action menu. All conservative: an action is only offered when the edit is provably correct
...
- **JUnit test support** — JUnit 4/5 tests appear in the Testing view (discovered statically — no build-tool execution), with run/debug profiles, gutter icons, per-test results, and failure messages. Runs go through the built-in debug adapter and the JUnit Platform Console Launcher (fetched once into `~/.m2` with consent + checksum verification); workspace-trust-gated because tests are project code
...
| `java-vsix-lite.diagnostics.unresolvedMembers` | `true` | Report an error for a member access when the receiver's full type hierarchy resolves but declares no such member, and for a bare variable reference with provably no declaration in scope. Conservative — stays silent whenever resolution is incomplete |
...
| `java-vsix-lite.diagnostics.styledHover` | `true` | Show this extension's diagnostics in the hover as a styled section (severity color, code-styled type names, clickable declaration link). While enabled, the declaration link appears only there, keeping the plain diagnostic block to one line |
...
| `java-vsix-lite.test.junitLauncherVersion` | `1.13.4` | Version of the JUnit Platform Console Launcher used to run tests (downloaded once into `~/.m2` with consent). Keep to the 1.x line for the Java 8+ baseline |
