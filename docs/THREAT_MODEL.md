...
| **Remote dependency fetch** (M6.2) | The Rust server stays network-free by construction — ALL HTTP lives in the TypeScript extension (Node's `https`, default TLS cert validation, no new npm dependency). Explicit command + Workspace Trust + one modal consent dialog per invocation; never automatic, never on activation, never retried in the background. Checksum verification is mandatory (prefer `.sha512`/`.sha256`, fall back `.sha1`; no sidecar at all ⇒ refused, never installed unverified) — this protects integrity (transport corruption / accidental mismatch), *not* provenance (a same-origin checksum published alongside a compromised artifact would still match). Download-to-temp-then-atomic-rename means a crash/cancel/checksum-failure never leaves a partial file at the real `~/.m2` path. Downloaded jars/poms are still only ever *data* (bytecode parser, XXE-safe XML parser) — nothing fetched is executed. Bounded fixed-point rebuild loop (max 5 rounds / 300 artifacts / 200MB per invocation). | M6.2. `editors/vscode/src/mavenFetch.ts`; lifecycle tests for the server-side `jvl/missingDependencies`/`jvl.classpath.rebuild` surfaces; manual E2E against real Maven Central (see task-22 report). |
| **Debugger launches project code** | Workspace Trust gate in `resolveDebugConfiguration` (session aborted before any process spawns); the JDK path is injected by the extension from the machine-scoped `java-vsix-lite.jdk.home` setting, overwriting anything workspace-provided; the debuggee is spawned argv-only with mandatory `-proc:none` on the auto-compile `javac`; JDWP is bound to 127.0.0.1 only; debuggee JDWP replies are treated as untrusted input (16 MiB packet cap, 64 KiB string caps, malformed data tears the session down). | TS trust tests (trusted + untrusted suites) + `crates/server/tests/dap.rs`. |
